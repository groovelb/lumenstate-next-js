...
- BrandValueCard: 브랜드 가치 카드. lucide-react 아이콘 + 제목 + 설명, FadeTransition cascade 지원 (`components/card/BrandValueCard.jsx`)
...
- LumenstateShell: 실제 `app/(shell)/layout.jsx`에 적용된 Lumenstate 전용 앱 셸. Day/Night 배경 블렌딩 + LumenstateGNB + Footer + FloatingTimeline. `CartProvider`, `TimelineProvider` 필요 (`components/layout/LumenstateShell.jsx`)
