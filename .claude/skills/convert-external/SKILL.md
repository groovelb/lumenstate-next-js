...
4. Three.js 유지: pnpm add three 필요 (사용자 확인) + next/dynamic({ ssr: false }) 래핑
5. default export → named export
6. Next.js client 경계: MUI/훅/이벤트 핸들러 사용 → 'use client' 추가
7. 정적 자산: Vite import → public/ 경로로 이동
```

**Client/Server 판단 (필수):**
- MUI import / 훅 / 이벤트 / 브라우저 API → `'use client'` 필수
- 데이터 fetch만 하는 순수 표시 컴포넌트 → 서버 컴포넌트 유지 (불필요한 `'use client'` 지양)
...
1. **파일 변환**
   ...
   - **Next.js client 경계 적용 (CRITICAL)**:
     - 클라이언트 패턴 감지 시 파일 최상단에 `'use client'` 추가
     - Vite 스타일 정적 import → `public/` 경로 문자열 사용
     - `import.meta.env.*` → `process.env.NEXT_PUBLIC_*`
     - `react-router-dom` → `next/link`, `next/navigation`
     - 클라이언트 전용 라이브러리(Three.js 등) → `next/dynamic({ ssr: false })`
...
2. **위치 결정**
   - `.claude/rules/directory-structure.md`에 따라 텍소노미 카테고리 디렉토리에 배치

3. **스토리 작성**
   - `component-work`의 `resources/storybook-writing.md` Read → 스토리 규칙 확인 (카테고리 매핑 포함)
