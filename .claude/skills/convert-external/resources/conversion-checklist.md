...
| `from 'three'` (Three.js) | → 사용자에게 `pnpm add three` 확인 + `next/dynamic({ ssr: false })` 래핑 |
| `from 'framer-motion'` | → 프로젝트에 이미 설치됨, 유지 가능 (`'use client'` 필수) |
| `from 'gsap'` | → 사용자에게 설치 확인, interactive-principles.md 참조 (`'use client'` 필수) |
| 기타 미설치 라이브러리 | → 사용자에게 의존성 추가 여부 확인 |

## Next.js 환경 변환 (CRITICAL)

App Router 환경에서 외부 코드를 변환할 때 추가로 감지해야 할 항목:

| 감지 패턴 | 변환 규칙 |
|-----------|----------|
| MUI import 있고 `'use client'` 없음 | → 파일 최상단에 `'use client'` 추가 |
| `useState`, `useEffect`, `useRef`, `useContext` 등 훅 | → `'use client'` 필수 |
| `onClick`, `onChange`, `onSubmit` 등 이벤트 핸들러 | → `'use client'` 필수 |
| `window`, `document`, `localStorage`, `navigator` 등 브라우저 API | → `'use client'` 필수 |
| Framer Motion, GSAP, Three.js 등 클라이언트 전용 라이브러리 | → `'use client'` 필수 |
| `import img from './foo.png'` (Vite 정적 import) | → `public/` 로 이동 + `'/foo.png'` 문자열 경로 |
| `import.meta.env.VITE_*` | → `process.env.NEXT_PUBLIC_*` |
| `import { Link } from 'react-router-dom'` | → `import Link from 'next/link'` |
| `useNavigate`, `useParams`, `useLocation` (react-router) | → `next/navigation`의 `useRouter`, `useParams`, `usePathname` |
| `<img src=...>` (정적 자산) | → `next/image` 권장 + width/height 명시 (CLS 방지) |
| 데이터 fetch만 하는 순수 표시 컴포넌트 | → `'use client'` 추가하지 말 것 (서버 컴포넌트 유지) |
| `opacity: 0` 애니메이션 초기 상태 | → `opacity: 0.01` 로 변경 (CLS/SEO 보호) |
| `width`, `height`, `top`, `left` 애니메이션 | → `transform` / `opacity` / `filter` 로 변경 |

### `'use client'` 배치 원칙

- 트리 끝단(leaf) 컴포넌트에만 배치 (CLAUDE.md `nextjs.md` 참조)
- 파일 **최상단** (import 위)에 위치
- 스토리 파일(`.stories.jsx`)에는 절대 사용 금지
