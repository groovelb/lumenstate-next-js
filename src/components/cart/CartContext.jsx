...
const CartContext = createContext({
  ...
  getSubtotal: () => 0,
  getItemCount: () => 0,
  getOptionLabel: (_key, value) => String(value ?? ''),
});
...
export const CartProvider = ({ children }) => {
  ...
  /**
   * 옵션 key/value 에 해당하는 사용자 친화적 라벨 조회.
   * PRODUCT_OPTIONS 에서 매칭되는 label 이 없으면 value 원문을 반환.
   *
   * @param {string} key - 옵션 키 (glassFinish / hardware / height ...)
   * @param {string} value - 옵션 값
   * @returns {string} 라벨
   */
  const getOptionLabel = useCallback((key, value) => {
    const list = PRODUCT_OPTIONS?.[key];
